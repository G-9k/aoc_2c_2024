...
void ej2a(
	...
	uint64_t tamaño = width * height;

	for(uint64_t i = 0; i < tamaño; i++){

		dst[i].r = funcionA(src[i].r, amount);
		dst[i].g = funcionA(src[i].g, amount);
		dst[i].b = funcionA(src[i].b, amount);
		dst[i].a = 255;
...
uint8_t funcionA(uint8_t color, uint8_t contrast){
	
	uint16_t res1 = contrast*(color - 128);
	uint8_t res2 = (res1 / 32)+128;

	if(res2 > 255) return 255;
	if(res2 < 0) return 0;
	else return res2;
